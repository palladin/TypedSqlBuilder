...
public class SqliteQueryTests
{
    ...
    public void FromWhereOrderBySelectNamed_GeneratesExpectedSql()
    {
        ...
        // Assert - SQLite uses || for string concatenation instead of CONCAT
        var expectedSql = "SELECT a0.Id AS Id, (a0.Name || :p2) AS prj0, (a0.Age + :p3) AS prj1 FROM customers a0 WHERE (a0.Age >= :p0) AND (a0.Name != :p1) ORDER BY a0.Name ASC";
    ...
    public void FromWhereAgeInSubqueryWithClosure_GeneratesCorrectSql()
    {
        ...
        Assert.Equal("SELECT * FROM customers a0 WHERE a0.Age IN (SELECT a1.Age AS Age FROM customers a1 WHERE a1.Name = (a0.Name || :p0))", sql);
        Assert.Single(parameters);
